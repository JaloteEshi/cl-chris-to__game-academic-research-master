...
Empirica.onRoundStart((game, round, players) => {
  console.log("onRoundStart", round.index + 1, "game", game._id, " started");
  // console.log("Round ---", round.stages);
...
Empirica.onStageStart((game, round, stage, players) => {
  // console.log('...onStageStart', round.index, stage._id, stage.index);
...
Empirica.onStageEnd((game, round, stage, players) => {
  // console.log('...onStageEnd', round.index, stage._id, stage.index);
  //this is not ideal, as we should compute it only once before the round outcome
  if (stage.name !== "outcome" && stage.name !== "setting") {
  ...
  } else if (stage.name == "setting") {
    try {
      //:random get round.setting
      const selectedPlayer = players[_.random(players.length - 1)];
      const roundSetting = selectedPlayer.get('round_setting');
      console.log('selected round setting is ', roundSetting);
      const { viewCount } = roundSetting;

      const playerSpeakTimeSetting = [];
      Object.keys(viewCount).map(_id => {
        const timeCount = viewCount[_id];
        const pl = players.find(p => {
          // console.log('-----', p, p.get('_id'), _id);
          return p._id == _id;
        });

        playerSpeakTimeSetting.push({ player: pl, timeCount });
      });
      const nextRound = game.rounds[round.index + 1];
      // :round have no addStage method
      // console.log('----', nextRound, game.rounds);
      if (nextRound) {
        wrapRoundCollector(nextRound, { game });
        setupRoundStage({ game, players, round: nextRound, playerSpeakTimeSetting });
      }
    } catch (err) {
      console.error('setting stage end error', err);
    }
  }
